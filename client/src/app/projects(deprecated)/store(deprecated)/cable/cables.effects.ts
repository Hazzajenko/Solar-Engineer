...
export class CablesEffects {
  ...
  /*  addPanel$ = createEffect(() =>
      this.actions$
        ...
        /!* this.store(deprecated).dispatch(BlocksStateActions.addBlockForGrid({block: action.panel})
   /!*      this.authService.login(action.credentials).pipe(
           map(user => AuthApiActions.loginSuccess({ user })),
           catchError(error => of(AuthApiActions.loginFailure({ error })))
         )*!/
       )
         mergeMap(() => this.store(deprecated).dispatch(BlocksStateActions.addBlockForGrid({block: action.}))/!* this.moviesService.getAll()
           .pipe(

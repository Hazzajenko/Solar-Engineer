...
/!*            combineLatestWith(this.panelsEntity.entities$.pipe(
          ...
          this.linksPathService.orderPanelsInLinkOrder(action.stringId).then((res) => {
            ...
            /!*    const stringPanels: PanelModel[] = panels.filter(
                ...
                const panelIds: string[] = stringPanels.map((p) => p.id)
                this.store(deprecated).dispatch(SelectedStateActions.setSelectedStringPanels({ panelIds }))
              ...
              `
                this.store(deprecated).dispatch(SelectedStateActions.setSelectedStringTooltip({ tooltip }))
      ...
      this.actions$.pipe(
        ...
        tap(async (action) => {
          ...
          if (selectedStringId) {
            ...
            /*this.store(deprecated).select(selectSelectedStringId).pipe(

    ...
    () =>
      this.actions$.pipe(
        ...
        map(async (action: any) => {
          ...
          if (panelBlocks) {
            await lastValueFrom(
              this.http.delete(`/api/projects(deprecated)/${projectId}/panels`, {
                body: {
          ...
          // },
          /*this.store(deprecated).select(selectAllBlocks).pipe(
              map((data: BlockModel[]) => {
                // let blocks: string[] = action.blockIds
                const blockIds: string[] = action?.blockIds
                const blocks: BlockModel[] = data?.filter((d) => blockIds?.includes(d.id))
                // const projectId = await firstValueFrom(this.store(deprecated).select(selectCurrentProjectId))
                     ...
                     this.observablesService
                       ...
                       .pipe(
                         switchMap((rails: RailModel[]) =>
                           this.http.delete(`/api/projects(deprecated)/${projectId}/rails`, {
                             body: {
                               rails,
                             },
                           }),

    ...
    if (!multiCreateState.locationStart) {
      ...
      firstValueFrom(
        this.store(deprecated)
          ...
          .pipe(combineLatestWith(this.store(deprecated).select(selectCurrentProjectId))),
        ...
        if (blocksInRoute.existingBlocks) {
          ...
          firstValueFrom(this.store(deprecated).select(selectSelectedStringId)).then((stringId) => {

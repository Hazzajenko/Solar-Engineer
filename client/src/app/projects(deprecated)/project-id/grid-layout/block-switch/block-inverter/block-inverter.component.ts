...
export class BlockInverterComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // this.panelToJoin$ = this.store(deprecated).select(selectPanelToJoin)
    this.selectedId$ = this.store.select(selectSelectedId).pipe(distinctUntilChanged())
    // this.selectedPositiveTo$ = this.store(deprecated).select(selectSelectedPositiveTo)
    // this.selectedNegativeTo$ = this.store(deprecated).select(selectSelectedNegativeTo)
    // this.selectedUnit$ = this.store(deprecated).select(selectUnitSelected)
    // this.selectedStringTooltip$ = this.store(deprecated).select(selectSelectedStringTooltip)
    // this.joinState$ = this.store(deprecated).select(selectJoinsState)
  }

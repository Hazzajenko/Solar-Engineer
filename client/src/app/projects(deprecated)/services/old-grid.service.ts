...
import { CableStateActions } from '../store(deprecated)/cable/cable.actions'
      ...
      this.http
        ...
        .subscribe({
          next: (envelope) => {
            ...
            // this.store(deprecated).dispatch(updateString({ string: envelope.string }));
            resolve(envelope)
          },
          error: (err) => {
            reject(err)

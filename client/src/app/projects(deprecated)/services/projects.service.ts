...
import { addInvertersByProjectId } from '../store(deprecated)/inverters/inverters.actions'
import { addTrackers } from '../store(deprecated)/trackers/trackers.actions'
import { addStringsByProjectId } from '../store(deprecated)/strings/strings.actions'
...
export class ProjectsService {
  ...
  /*    private store(deprecated): Store<AppState>,
        private inverters: InvertersService*/

  /*  getUserProjects(): Observable<ProjectsEnvelope> {
      return this.http.get<ProjectsEnvelope>(environment.apiUrl + '/projects(deprecated)');
    }*/
          ...
          next: (envelope) => {
            ...
            /*            if (envelope.stringsByProjectId) {
                          ...
                          envelope.stringsByProjectId.map((stringModel) => {
                            this.store(deprecated).dispatch(addString({ stringModel }));
                          });
                        }*/
  ...
  /*
    getProjectById(request: ProjectModel): Promise<ProjectModel> {
      return new Promise<ProjectModel>((resolve, reject) =>
        this.http
          .get<ProjectModel>(environment.apiUrl + `/projects(deprecated)/${request.id}`)
          .subscribe({
            next: (project) => {
              this.store(deprecated).dispatch(selectProject({ project }));
              this.inverters

...
export const selectStringPanelsState =
  createFeatureSelector<State.StringPanelState>('stringPanels');
...
export const selectAllStringPanels = createSelector(
  selectStringPanelsState,
  State.selectAll
);
...
export const selectStringPanelsByStringIdRouteParams = createSelector(
  ...
  (stringPanels, { stringId }) =>
    stringPanels.filter(
      (stringPanel) => stringPanel.stringId === Number(stringId)
    )
);

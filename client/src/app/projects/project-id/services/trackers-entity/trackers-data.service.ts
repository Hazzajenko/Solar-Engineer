...
export class TrackersDataService extends DefaultDataService<TrackerModel> {
  ...
  override add(entity: TrackerModel): Observable<TrackerModel> {
    return this.http
      .post<CreateTrackerResponse>(
        environment.apiUrl + `/projects/3/tracker`,
        entity,
      )
  ...
  override update(update: Update<TrackerModel>): Observable<TrackerModel> {
    console.log(update)
    return this.http
      .put<UpdateTrackerResponse>(
        environment.apiUrl + `/projects/3/tracker/${update.id}`,
        update,
      )
      .pipe(map((res) => res.tracker))
  }
  ...
  override delete(key: number | string): Observable<number | string> {
    return this.http
      .delete<DeleteTrackerResponse>(
        environment.apiUrl + `/projects/3/tracker/${key}`,
      )

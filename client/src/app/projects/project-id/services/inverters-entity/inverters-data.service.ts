...
export class InvertersDataService extends DefaultDataService<InverterModel> {
  ...
  override add(entity: InverterModel): Observable<InverterModel> {
    return this.http
      .post<CreateInverterResponse>(
        environment.apiUrl + `/projects/3/inverter`,
        entity,
      )
  ...
  override update(update: Update<InverterModel>): Observable<InverterModel> {
    console.log(update)
    return this.http
      .put<UpdateInverterResponse>(
        environment.apiUrl + `/projects/3/inverter/${update.id}`,
        update,
      )
      .pipe(map((res) => res.inverter))
  }
  ...
  override delete(key: number | string): Observable<number | string> {
    return this.http
      .delete<DeleteInverterResponse>(
        environment.apiUrl + `/projects/3/inverter/${key}`,
      )

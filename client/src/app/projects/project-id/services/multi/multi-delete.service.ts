  ...
  multiDelete(location: string) {
    firstValueFrom(this.store.select(selectMultiState)).then((multiState) => {
      if (!multiState.locationStart) {
        ...
        const blocksInBox = getBlocksInBox(multiState.locationStart!, location)
        console.log('blocksInBox', blocksInBox)
        /*        const blocksInPath = getLocationsFrom2(multiState.locationStart, location)
                console.log(blocksInPath)*/
        if (blocksInBox) {
          firstValueFrom(this.store.select(selectBlocksByProjectIdRouteParams)).then((blocks) => {
            console.log(blocks)
            const blocksToDelete = blocks
              .filter((block) => blocksInBox.includes(block.location!))
              .map((block) => block.id)
            console.log(blocksToDelete)
            this.store.dispatch(
              BlocksStateActions.deleteManyBlocksForGrid({ blocks: blocksToDelete }),
            )
            /*    this.store.dispatch(
                  BlocksStateActions.deleteManyBlocksForGrid({ blocks: blocksToDelete }),
                )*/
        ...
        /*        if (blocksInPath) {
                  firstValueFrom(this.store.select(selectBlocksByProjectIdRouteParams)).then((blocks) => {
                    const blocksToDelete = blocks
                      .filter((block) => blocksInPath.includes(block.location))
                      .map((block) => block.id)
                    this.store.dispatch(
                      BlocksStateActions.deleteManyBlocksForGrid({ blocks: blocksToDelete }),
                    )

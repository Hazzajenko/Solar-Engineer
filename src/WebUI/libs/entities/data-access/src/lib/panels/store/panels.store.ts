...
import { getPanelNearbyInLinkModeExcludingOne } from '@entities/utils'
...
function panelsStoreFactory(store: Store) {
	...
	const select = {
		allPanels,
		getById: (id: PanelId) => entities()[id],
		getByIdOrUndefined: (id: PanelId | undefined) => (id ? entities()[id] : undefined),
		getByIds: (ids: PanelId[]) => ids.map((id) => entities()[id]).filter(isNotNull),
		getByStringId: (stringId: string) => allPanels().filter((panel) => panel.stringId === stringId),
		getPanelUnderMouse: (point: TransformedPoint) =>
			allPanels().find((entity) => isPointInsideEntity(point, entity)),
		getNearbyPanelInLinkModeExcludingOne: (point: TransformedPoint, panelId: PanelId) =>
			getPanelNearbyInLinkModeExcludingOne(allPanels(), point, panelId),
	}

	const dispatch = {
		loadPanels: (panels: PanelModel[]) => store.dispatch(PanelsActions.loadPanels({ panels })),
		addPanel: (panel: PanelModel) => store.dispatch(PanelsActions.addPanel({ panel })),
		...
		deletePanel: (panelId: PanelId) => store.dispatch(PanelsActions.deletePanel({ panelId })),
		deleteManyPanels: (panelIds: PanelId[]) =>
			store.dispatch(PanelsActions.deleteManyPanels({ panelIds })),
		clearPanelsState: () => store.dispatch(PanelsActions.clearPanelsState()),
	}

	return {
		select,
		dispatch /*	get allPanels$() {
		 return allPanels$
		 },
		 get allPanels() {

...
import { PanelsStoreService } from 'deprecated/design-app/feature-panel'
import { SelectedStoreService } from 'deprecated/design-app/feature-selected'
import { StringsFactoryService } from 'deprecated/design-app/feature-string'
...
export class KeyUpDirective implements OnInit {
	...
	ngOnInit(): void {
		this._renderer.listen(window, 'keyup', (event: KeyboardEvent) => {
			...
			this.onKeyUpHandler(event).catch(console.error)
		})
		// EventName.Ke
	}
	...
	private async onKeyUpHandler(event: KeyboardEvent) {
		...
		switch (event.key) {
			...
			case KEYS.G: {
				...
				const biggestNearByEntityArray =
					await this._objectPositioningService.selectBiggestNearByEntityArray(ENTITY_TYPE.Panel)
				if (biggestNearByEntityArray.ids.length > 1) {
					return this._objectPositioningService.moveGroupOfPanelsToSameAxisPositionV2(
						biggestNearByEntityArray.ids,
						biggestNearByEntityArray.direction,
					)
				}
				return

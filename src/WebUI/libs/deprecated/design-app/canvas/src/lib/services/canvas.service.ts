...
import { PanelBackgroundColor, PanelStylerService } from 'deprecated/design-app/feature-panel'
import { SelectedStoreService } from 'deprecated/design-app/feature-selected'

@Injectable({
	providedIn: 'root',
})
export class CanvasService extends BlockRectHelpers {
	...
	drawLinesForBlocks(id: string) {
		...
		const gridBlockRects = this._componentElementService.elements.map((e) =>
			this.getBlockRectFromElement(e),
		)
	...
	drawLineForDirection(
		...
		const distanceToClosestPanel = this.getDistanceToClosestPanel(
			blockRect,
			closestPanelRect,
			lineDirection,
		)
		...
		if (distanceToClosestPanel / this._viewPositioningService.scale < 100) {
			this._panelStylerService.setBackgroundColorForPanelById(
				closestPanelRect.id,
				PanelBackgroundColor.Nearby,
				lineDirection,
			)
		} else {
			this._panelStylerService.setBackgroundColorForPanelById(
				closestPanelRect.id,
				PanelBackgroundColor.LineThrough,
				lineDirection,
			)
		}
		...
		panelsWithinDistance.forEach((panelRect) => {
			if (this.canvasConfig.showGridAxisLines) {
				...
				gridMoveToPoints.moveToPointTop =
					this._mousePositioningService.getMousePositionFromXYForCanvas(
						gridMoveToPoints.moveToPointTop,
					)
	...
	protected strokeTwoPoints(moveToPoint: Point, lineToPoint: Point) {
		const { x: moveToX, y: moveToY } = this._mousePositioningService.applyScaleToPoint(moveToPoint)
		const { x: lineToX, y: lineToY } = this._mousePositioningService.applyScaleToPoint(lineToPoint)
		this.ctx.beginPath()

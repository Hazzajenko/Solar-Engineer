...
export class DesignLayoutDirective
  extends DesignLayoutService
  implements OnInit {
  ...
  private onMouseUpHandler(event: MouseEvent) {
    ...
    if (this.isAltDragging) {
      ...
      const x = event.pageX - this._element.offsetLeft * this.scale
      const y = event.pageY - this._element.offsetTop * this.scale
      const entitiesInSelectionBox = this._objectPositioningService.getAllElementsBetweenTwoPoints(this.canvasStartPoint, { x, y })

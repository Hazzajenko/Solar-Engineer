...
export class ObjectPositioningService {
	...
	public getElementRectPositionOffScrollById(elementId: string) {
		...
		const x =
			(entityRect.x - scrollRect.left + entityRect.width) / this._mousePositioningService.scale
		const y =
			(entityRect.y - scrollRect.top + entityRect.height) / this._mousePositioningService.scale
	...
	public moveGroupOfPanelsToSameAxisPositionV2(ids: string[], direction: LineDirection) {
		...
		const axis = direction === LineDirection.Left || direction === LineDirection.Right ? 'x' : 'y'
		const min = Math.min(...rects.map((r) => r[axis]))
		const max = Math.max(...rects.map((r) => r[axis]))
		const middle = (min + max) / 2
		const offset = middle - rects[0][axis]
		console.log('moveGroupOfPanelsToSameAxisPositionV2', {
			ids,
			direction,
			rects,
			axis,

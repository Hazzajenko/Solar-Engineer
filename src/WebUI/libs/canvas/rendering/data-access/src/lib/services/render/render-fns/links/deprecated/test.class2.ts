	...
	public draw(ctx: CanvasRenderingContext2D): void {
		...
		for (let t = 0; t <= 1; t += 0.01) {
			const x =
				...
				3 * Math.pow(1 - t, 2) * t * this.p2.x +
				3 * (1 - t) * Math.pow(t, 2) * this.p3.x +
				Math.pow(t, 3) * this.p4.x
			const y =
				Math.pow(1 - t, 3) * this.p1.y +
				3 * Math.pow(1 - t, 2) * t * this.p2.y +
				3 * (1 - t) * Math.pow(t, 2) * this.p3.y +
				Math.pow(t, 3) * this.p4.y
			ctx.lineTo(x, y)
		}
		ctx.stroke()
	}

	public curveThroughPoints(points: Point[]): void {
		if (points.length < 4) {
			throw new Error('At least 4 points are required to create a cubic Bezier curve')
		}

		this.p1 = points[0]
		this.p4 = points[points.length - 1]

		const dx = points[points.length - 1].x - points[0].x
		const dy = points[points.length - 1].y - points[0].y

		const p2x = points[0].x + dx / 3
		const p2y = points[0].y

		const p3x = points[points.length - 1].x - dx / 3
		const p3y = points[points.length - 1].y

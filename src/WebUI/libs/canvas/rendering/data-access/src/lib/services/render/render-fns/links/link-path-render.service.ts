/*
 ...
 export class LinkPathRenderService {
 ...
 const groupedByLinkChainSorted = groupedByLinkChain.map((chain) => sortPanelLinks(chain))
 ...
 this.drawLinkChainCurvedPath(ctx, linkChain)
 })
 // const
 // const groupedByLinkChain = groupLinkPointsByChain(linksInOrder)
 /!*		groupedByLinkChain.forEach((linkChain, index) => {
 const sorted = sortPanelLinks(linkChain)
 const pointsToNumberArray = reduceLinkPointsToNumberArray(sorted)
 this.drawLinkChainCurvedPath(ctx, pointsToNumberArray)
 })*!/
 /!*
 ...
 private drawLinkChainCurvedPath(ctx: CanvasRenderingContext2D, points: number[]) {
 ...
 /!*const drawLinkModePathLinesCurved = (
 ctx: CanvasRenderingContext2D,
 customEntities: CanvasEntity[] | undefined,
 linksInOrder: PanelLinkModel[],
 selectedPanelLinkId: string | undefined,

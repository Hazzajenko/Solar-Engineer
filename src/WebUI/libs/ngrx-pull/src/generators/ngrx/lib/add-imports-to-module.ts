...
function addRootEffectsImport(
	...
	if (isParentStandalone) {
		...
		sourceFile = addImportToModule(tree, sourceFile, parentPath, effectsForEmptyRoot)
	}
...
function addStoreForFeatureImport(
	...
	if (isParentStandalone) {
		...
		sourceFile = addImportToModule(tree, sourceFile, parentPath, storeForFeature)
	}
	return sourceFile
}
...
function addEffectsForFeatureImport(
	...
	if (isParentStandalone) {
		...
		sourceFile = addImportToModule(tree, sourceFile, parentPath, effectsForFeature)
	}
	return sourceFile
}

export function addImportsToModule(tree: Tree, options: NormalizedNgRxGeneratorOptions): void {
	if (!tsModule) {
	...
	const addImport = (
		...
		return insertImport(tree, source, parentPath, symbolName, fileName, isDefault)
	}

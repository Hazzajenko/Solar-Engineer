            ...
            // var invitedMembers = new List<GroupChatMemberDto>();
        {
            foreach (var requestInvite in request.CreateGroupChatRequest.Invites)
            {
                ...
                var isAppUserGroupChatExisting =
                    await _unitOfWork.AppUserGroupChatsRepository.GetByAppUserAndGroupChatIdAsync(invitedUserId,
                        groupChatId);
            ...
            var memberIds = await _unitOfWork.AppUserGroupChatsRepository.GetGroupChatMemberIdsAsync(groupChatId);

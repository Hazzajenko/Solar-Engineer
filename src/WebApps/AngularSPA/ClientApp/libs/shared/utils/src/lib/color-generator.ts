...
export function generateDifferentColor2(colors: Array<[number, number, number]>): [number, number, number] {
  const [newR, newG, newB] = generateVibrantColor()
...
export function getComplementaryColor(r: number, g: number, b: number): string {
  return '#' + (0x1000000 + (255 - r) * 0x10000 + (255 - g) * 0x100 + (255 - b)).toString(16).substring(1)
}

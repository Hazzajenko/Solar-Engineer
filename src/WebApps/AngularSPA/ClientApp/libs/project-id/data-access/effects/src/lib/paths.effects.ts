...
import { LinksService, PathsService } from '@project-id/data-access/api'
...
export class PathsEffects {
  ...
  addPathHttp$ = createEffect(() =>
      this.actions$.pipe(
        ofType(PathsActions.addPath),
        switchMap(({ path }) => this.projectsStore.select.isWebWithProject$.pipe(
          switchMap(
            ([isWeb, project]) => {
  ...
  deletePathHttp$ = createEffect(() =>
      this.actions$.pipe(
        ...
        switchMap(({ pathId }) => this.projectsStore.select.isWebWithProject$.pipe(
          switchMap(
            ([isWeb, project]) => {
              if (!project) return of(undefined)
              if (isWeb) {

...
export class PanelStylerService extends PanelStylerFields {
	...
	public addNearbyPanel(designRectModel: DesignRectModel, direction: LineDirection) {
		...
		if (this.nearbyPanels.map((x) => x.id).includes(nearbyPanel.id)) {
			return
		}
		this.nearbyPanels = [...this.nearbyPanels, nearbyPanel]
	...
	setBackgroundColorForPanelById(
		panelId: string,

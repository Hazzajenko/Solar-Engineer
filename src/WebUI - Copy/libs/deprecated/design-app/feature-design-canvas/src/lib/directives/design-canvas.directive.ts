 ...
 *!/
 ...
 const inSingleToMoveState = this._machine.snapshot.matches('ToMoveState.SingleMoveInProgress')
 if (entityUnderMouse && !inSingleToMoveState) {
 ...
 // if (entityUnderMouse) {
 // 	const entity = this._state.entities.canvasEntities.getEntityById(entityUnderMouse.id)
 // 	assertNotNull(entity)
 this._objPositioning.singleToMoveMouseMove(event)
 console.log('singleToMoveMouseMove')
 return
 // }
 }

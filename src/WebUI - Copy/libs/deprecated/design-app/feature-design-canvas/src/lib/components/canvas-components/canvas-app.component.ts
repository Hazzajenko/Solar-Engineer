...
@Component({
	...
	template: `
		<app-canvas #app>
			<app-ctx-panel [ctx]="app.ctx" [panel]="panel" *ngFor="let panel of panels"></app-ctx-panel>
		</app-canvas>
	`,
	standalone: true,
	imports: [CanvasComponent, CtxPanelComponent, NgForOf],
})
export class CanvasAppComponent implements OnInit {
	@ViewChild(CanvasComponent) private _canvas!: CanvasComponent
	@ViewChildren(CtxPanelComponent) private _ctxPanels: QueryList<CtxPanelComponent> =
		new QueryList<CtxPanelComponent>()

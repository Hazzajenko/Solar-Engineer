...
import {
	...
	selectCanvasEntitiesViaDictionary,
...
export class CanvasEntitiesQueries {
	...
	private readonly _entityDictionary$: Observable<Dictionary<CanvasEntity>> = this._store.pipe(
		select(selectCanvasEntitiesViaDictionary),
	)
	private readonly _entities$: Observable<CanvasEntity[]> = this._store.pipe(
		select(selectAllCanvasEntities),
	)

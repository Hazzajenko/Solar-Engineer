...
import { EntitiesActions, GridStringsActions, ProjectsHubService, StringsService } from '../'
...
export class GridStringsEffects
  extends BaseService {
  ...
  initLocalStrings$ = createEffect(() =>
    this.actions$.pipe(
      ...
      map(({ localProject }) =>
        GridStringsActions.loadStringsSuccess({ strings: localProject.strings }),
      ),
    ),
  )
  ...
  addStringEntity$ = createEffect(() =>
    this.actions$.pipe(
      ofType(GridStringsActions.addString),

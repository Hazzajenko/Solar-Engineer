...
export class DisconnectionPointsDataService extends DefaultDataService<DisconnectionPointModel> {
  ...
  override getAll(): Observable<DisconnectionPointModel[]> {
    return this.http
      .get<GetDisconnectionPointsResponse>(`/api/projects/3/disconnection-points`)
      .pipe(map((res) => res.disconnection_points))
  }

  override add(entity: DisconnectionPointModel): Observable<DisconnectionPointModel> {
    return this.http
      .post<CreateDisconnectionPointResponse>(`/api/projects/3/disconnection-point`, entity)
      .pipe(map((res) => res.disconnection_point))
  }

  override update(update: Update<DisconnectionPointModel>): Observable<DisconnectionPointModel> {
    console.log(update)
    return this.http
  ...
  override delete(key: number | string): Observable<number | string> {
    return this.http
      .delete<DeleteDisconnectionPointResponse>(`/api/projects/3/disconnection-point/${key}`)

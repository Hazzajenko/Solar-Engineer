...
import { ChangeDetectionStrategy, Component, inject, Input, NgZone, OnInit, ViewChild } from '@angular/core'
...
@Component({
  selector: 'app-chatroom-conversation-component',
  templateUrl: './chatroom-conversation.component.html',
  styles: [`
    html,
    body {
      height: 100%;
...
export class ChatroomConversationComponent implements OnInit {
  ...
  @Input() set recipientImport(recipient: string | undefined) {
    ...
    this.conversationMessages$ = this.messagesStore.select.messagesWithUser$(recipient)

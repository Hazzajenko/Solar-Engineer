...
export class CablesDataService extends DefaultDataService<CableModel> {
  ...
  override add(entity: CableModel): Observable<CableModel> {
    return this.http
      .post<CreateCableResponse>(environment.apiUrl + `/projects/3/cable`, entity)
  ...
  override update(update: Update<CableModel>): Observable<CableModel> {
    ...
    console.log(update)
    return this.http
      .put<UpdateCableResponse>(environment.apiUrl + `/projects/3/cable/${update.id}`, update)
      .pipe(map((res) => res.cable))
  ...
  override delete(key: number | string): Observable<number | string> {
    return this.http
      .delete<DeleteCableResponse>(environment.apiUrl + `/projects/3/cable/${key}`)

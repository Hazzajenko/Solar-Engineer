...
export class InvertersService {
  ...
  getInvertersByProjectId(projectId: number): Promise<InvertersEnvelope> {
    return new Promise<InvertersEnvelope>((resolve, reject) =>
      this.http.get<InvertersEnvelope>(environment.apiUrl + `/projects/${projectId}`).subscribe({
        next: (envelope) => {
  ...
  deleteInverter(projectId: number, inverterId: number): Promise<InverterEnvelope> {
    return new Promise<InverterEnvelope>((resolve, reject) =>
      this.http
        .delete<InverterEnvelope>(`${environment.apiUrl}/projects/${projectId}/inverters`, {
          body: {
            id: inverterId,
          },
        })

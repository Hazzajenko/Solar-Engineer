...
export const trackerAdapter: EntityAdapter<TrackerModel> = createEntityAdapter<TrackerModel>({
...
export const trackersReducer = createReducer(
  ...
  on(TrackerActions.addTracker, (state, { tracker }) => trackerAdapter.addOne(tracker, state)),

  on(TrackerActions.addTrackers, (state, { trackers }) => trackerAdapter.addMany(trackers, state)),
...
export const { selectIds, selectEntities, selectAll } = trackerAdapter.getSelectors()

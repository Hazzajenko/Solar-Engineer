...
export class JoinNearbyBlocksPipe implements PipeTransform {
  ...
  transform(cable: CableModel): SurroundingModel {
    ...
    const data = combineLatest([this.joinsEntity.entities$, this.cablesEntity.entities$]).subscribe(
      ([joins, cables]) => {
        ...
        if (findTop) {
          const cable = cables.find((cable) => cable.location === findTop.location)
          if (cable) {
            if (cable.in_join) {
        ...
        if (findBottom) {
          const cable = cables.find((cable) => cable.location === findBottom.location)
          if (cable) {
            if (cable.in_join) {
              const cablesInJoin = cables.filter((getCable) => getCable.join_id === cable.join_id)
              cablesInJoin.forEach((cableInJoin) => {
        ...
        if (findRight) {
          const cable = cables.find((cable) => cable.location === findRight.location)
          if (cable) {
            if (cable.in_join) {
              const cablesInJoin = cables.filter((getCable) => getCable.join_id === cable.join_id)
              cablesInJoin.forEach((cableInJoin) => {
